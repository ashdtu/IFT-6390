...
def ones_at_the_end(x):
    ...
    """
    x_arr = np.array(x)
    all_ones = list(x_arr[x_arr == 1])
    not_ones = list(x_arr[x_arr != 1])
    return not_ones[::-1] + all_ones
...
def final_position(instructions):
    ...
    start = np.array([0, 0])
    for item in split_instructions:
        start += np.array(direction[item])
...
def data_normalization(X):
    ...
    normalised_data = (data - mean_data) / std_data
    labels = X[:, -1]
    return np.concatenate((normalised_data, labels[:, None]), axis=1)
...
def entropy(p):
    ...
    """
    p_sum = np.sum(p)
    all_positive  = np.sum(p >= 0)
    if all_positive == len(p) and p_sum == 1:
        entropy = [-x*np.log2(x) if x!=0 else 0 for x in p]
...
def heavyball_optimizer(x, inputs, alpha=0.9, beta=0.1):
    ...
    for i, gradient_vec in enumerate(inputs):
        x_updated = x_new - alpha * gradient_vec + beta * (x_new - x_old)
        x_old = x_new
        x_new = x_updated
...
class NearestCentroidClassifier:
    ...
    def fit(self, X, y):  # question A
        ...
        for label_val in range(self.k):
            valid_points = X[y == label_val]
            centroid = np.mean(valid_points, axis=0)
            self.centroids[label_val] = centroid
    ...
    def predict(self, X):  # question B
        ...
        for i in range(X.shape[0]):
            distances = np.linalg.norm(X[i] - self.centroids)
